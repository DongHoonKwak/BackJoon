package exam.mjm.android.network.activity;
...
public class ExamNetworkActivity_01 extends Activity implements OnClickListener
{
	...
	public void onClick(View v) 
	{
		...
		try 
		{
			String html = loadKmaData(); //xml데이터
	...
	private String loadKmaData() throws Exception 
	{
		...
		urlConnection.setConnectTimeout(10000);//해당 웹 페이지 연결시간을 10초로 설정
		urlConnection.setUseCaches(false);//매번 서버에서 읽어오기
		
		StringBuilder sb = new StringBuilder();//고속 문자열 결합체 //stringbuffer랑 같음
		
		if (urlConnection.getResponseCode() == HttpURLConnection.HTTP_OK)  //연결
		{
